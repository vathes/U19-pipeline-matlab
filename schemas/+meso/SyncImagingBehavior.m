...
-> meso.FieldOfView
---
sync_im_frame                      :    blob@mesoimaging   # frame number within tif file
sync_im_frame_global               :    blob@mesoimaging   # global frame number in scan
sync_behav_block_by_im_frame       :    blob@mesoimaging   # array with behavioral block for each imaging frame
sync_behav_trial_by_im_frame       :    blob@mesoimaging   # array with behavioral trial for each imaging frame
sync_behav_iter_by_im_frame        :    blob@mesoimaging   # array with behavioral trial for each imaging frame, some extra zeros in file 1, marking that the behavior recording hasn't started yet.
sync_im_frame_span_by_behav_block  :    blob@mesoimaging   # cell array with first and last imaging frames for for each behavior block
sync_im_frame_span_by_behav_trial  :    blob@mesoimaging   # cell array with first and last imaging frames for for each behavior trial
sync_im_frame_span_by_behav_iter   :    blob@mesoimaging   # cell array with first and last imaging frames for for each behavior iteration within each trial
%}
